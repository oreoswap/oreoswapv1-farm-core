...
contract("Timelock", ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.oreo = await OreoToken.new({ from: alice });
    this.timelock = await Timelock.new(bob, "28800", { from: alice }); //8hours
  });

  it("should not allow non-owner to do operation", async () => {
    await this.oreo.transferOwnership(this.timelock.address, { from: alice });
    await expectRevert(
      this.oreo.transferOwnership(carol, { from: alice }),
      "Ownable: caller is not the owner"
    );
    await expectRevert(
      this.oreo.transferOwnership(carol, { from: bob }),
      "Ownable: caller is not the owner"
    );
    await expectRevert(
  ...
  it("should do the timelock thing", async () => {
    ...
    assert.equal((await this.oreo.owner()).valueOf(), carol);
  });

  it("should also work with PastryChef", async () => {
    ...
    await this.oreo.transferOwnership(this.chef.address, { from: alice });
