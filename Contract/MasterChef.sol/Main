...
import '@oreoswap/oreo-swap-lib/contracts/access/Ownable.sol';
...
contract MasterChef is Ownable {
    ...
    function pendingOreo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 oreoReward = multiplier.mul(cakePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accOreoShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
