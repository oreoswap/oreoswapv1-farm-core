...
// MilkBar with Governance.
contract MilkBar is BEP20('MilkBar Token', 'MILK') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (cookieschef).
    ...
    // Safe oreo transfer function, just in case if rounding error causes pool to not have enough CAKEs.
